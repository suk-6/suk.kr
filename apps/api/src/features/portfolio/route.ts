...
import { getNotices } from "../notices/repo";
...
export const portfolioRoute = new Hono<{ Bindings: Env }>().get(
	"/",
	async (context) => {
		const [settings, projects, entries, skills, notices] = await Promise.all([
			getSettings(context.env.DB),
			getProjects(context.env.DB),
			getEntries(context.env.DB),
			getSkills(context.env.DB),
			getNotices(context.env.DB),
		]);
		return context.json({ settings, projects, entries, skills, notices });
	},
);
